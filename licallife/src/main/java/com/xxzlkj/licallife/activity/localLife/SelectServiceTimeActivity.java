 ...
 *         2017/3/21 14:48
 */
public class SelectServiceTimeActivity extends MyBaseActivity {
    ...
    private void setData(String serverTimeStyle, List<ScheduleBean> schedule, ScheduleBean.TimeBean selectTimeBean) {
        ...
        for (int i = 0; i < schedule.size(); i++) {
            ...
            // 设置内容
            fragmentList.add(SelectServiceTimeTabFragment.newInstance(extras, scheduleBean.getTime(),selectTimeBean));// extras里面含有字段：服务时间样式
        }
        // 通知改变
    ...
    public static Intent newIntent(Context context, String serverTimeStyle, Serializable desDataBean, int serviceMinutesNumber, long selectedTime) {
        Intent intent = new Intent(context, SelectServiceTimeActivity.class);
