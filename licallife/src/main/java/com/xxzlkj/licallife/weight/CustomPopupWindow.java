package com.xxzlkj.licallife.weight;
...
import com.xxzlkj.shop.weight.WheelView;
...
public class CustomPopupWindow extends BasePopupWindow {
    ...
    public CustomPopupWindow(Activity activity, OnMyClickListener listener) {
        super(activity);
        createPopupWindow(activity, listener);
    }
    ...
    public CustomPopupWindow(Activity activity, OnMyClickListener listener, List<String> ymds, List<String> hms) {
        super(activity);
        createPopupWindow(activity, listener, ymds, hms);
    }

    public CustomPopupWindow(Activity activity, int type, List<ScheduleBean> bean) {
        super(activity);
        createPopupWindow(activity, type, bean);
    }
     ...
     * @param hms
     */
    public void createPopupWindow(final Activity activity, final OnMyClickListener listener, List<String> ymds, List<String> hms) {
        LayoutInflater inflater = (LayoutInflater) activity.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View viewType = inflater.inflate(R.layout.popup_wiondow_time_wheel, null);
        final WheelView mLeftWheelView = (WheelView) viewType.findViewById(R.id.id_ww_wheel_left);
        final WheelView mRightWheelView = (WheelView) viewType.findViewById(R.id.id_ww_wheel_right);
        ...
        viewType.findViewById(R.id.id_ww_sure).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                CustomPopupWindow.this.dismiss();
                if (listener == null) return;
                listener.sureClickListener(mLeftWheelView.getSeletedItem() + " " + mRightWheelView.getSeletedItem());

            }
        });

        mLeftWheelView.setItems(ymds);
        mRightWheelView.setItems(hms);
     ...
     * 微信分享
     */
    public void createPopupWindow(final Activity activity, final OnMyClickListener listener) {
        LayoutInflater inflater = (LayoutInflater) activity.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View viewType = inflater.inflate(R.layout.popup_wiondow_share, null);

        viewType.findViewById(R.id.id_share_cancle).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                setBgAlpha(activity, 1f);
                CustomPopupWindow.this.dismiss();
        ...
        viewType.findViewById(R.id.id_wechat_friend).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                setBgAlpha(activity, 1f);
                CustomPopupWindow.this.dismiss();
                if (listener == null) return;
                listener.sureClickListener("0");

            }
        });
        viewType.findViewById(R.id.id_wechat_friend_circle).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                setBgAlpha(activity, 1f);
                CustomPopupWindow.this.dismiss();
                if (listener == null) return;
                listener.sureClickListener("1");
     ...
     * 查看最近可约
     */
    public void createPopupWindow(final Activity activity, int type, List<ScheduleBean> timeBeanList) {
        LayoutInflater inflater = (LayoutInflater) activity.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View view = inflater.inflate(R.layout.dialog_schedule, null);
        // 标题
        TextView mTitleTextView = (TextView) view.findViewById(R.id.id_title);
        // 关闭
        view.findViewById(R.id.id_close).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                dismiss();
            }
        ...
        // 时间刻度
        LinearLayout mTimeLinearLayout = (LinearLayout) view.findViewById(R.id.id_time_layout);
        // 可约时间列表
        RecyclerView mScheduleRecyclerView = (RecyclerView) view.findViewById(R.id.id_schedule_recyclerview);
        mScheduleRecyclerView.setLayoutManager(new LinearLayoutManager(activity.getApplicationContext()));
        mScheduleRecyclerView.setNestedScrollingEnabled(false);
        ScheduleAdapter mScheduleAdapter = new ScheduleAdapter(activity.getApplicationContext(), R.layout.adapter_schedule_list_item, type);
        mScheduleRecyclerView.setAdapter(mScheduleAdapter);
        ScheduleUtil.drawingScale(activity.getApplicationContext(), mTimeLinearLayout, type);

        if (timeBeanList != null && timeBeanList.size() > 0) {
            mScheduleAdapter.addList(timeBeanList);
            switch (type) {
                case 1:// 每天
                    mTitleTextView.setText("最近" + timeBeanList.size() + "天可约时间");
                    break;
                case 2:// 每月
                    TextView mTextView = (TextView) view.findViewById(R.id.id_time);
                    mTextView.setText("日期");
                    mTitleTextView.setText("最近" + timeBeanList.size() + "月可约时间");
                    break;
            }
        }
