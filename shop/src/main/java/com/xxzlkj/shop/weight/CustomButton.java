...
import com.xxzlkj.shop.utils.FontUtil;
...
public class CustomButton extends LinearLayout{
    ...
    private int mDefaultTextSize 				= PixelUtil.sp2px(getContext(), 15);
    private int mDefaultTextGravity             = 0x11; // Gravity.CENTER
    private String mText 						= null;

    // # Icon Attributes
    private Drawable mIconResource 				= null;
    private int  mFontIconSize 					= PixelUtil.sp2px(getContext(), 15);
    ...
    public CustomButton(Context context, AttributeSet attrs){
        ...
        TypedArray attrsArray 	= context.obtainStyledAttributes(attrs, R.styleable.CustomButton, 0, 0);
    ...
    private TextView setupTextView(){
        ...
        textView.setTextSize(PixelUtil.px2sp(getContext(), mDefaultTextSize));
    ...
    private TextView setupFontIconView(){
        ...
        if(mFontIcon!=null){
            ...
            if(!isInEditMode()){
                fontIconView.setTextSize(PixelUtil.px2sp(getContext(), mFontIconSize));
...
//                iconViewParams.leftMargin = 10;
    ...
    private void initAttributesArray(TypedArray attrsArray){
        ...
        mDefaultBackgroundColor 		= attrsArray.getColor(R.styleable.CustomButton_fb_defaultColor,mDefaultBackgroundColor);
        mFocusBackgroundColor 			= attrsArray.getColor(R.styleable.CustomButton_fb_focusColor,mFocusBackgroundColor);
        mDisabledBackgroundColor        = attrsArray.getColor(R.styleable.CustomButton_fb_disabledColor, mDisabledBackgroundColor);

        mEnabled                        = attrsArray.getBoolean(R.styleable.CustomButton_android_enabled, true);

        mDisabledTextColor              = attrsArray.getColor(R.styleable.CustomButton_fb_disabledTextColor, mDisabledTextColor);
        mDisabledBorderColor            = attrsArray.getColor(R.styleable.CustomButton_fb_disabledBorderColor, mDisabledBorderColor);
        mDefaultTextColor 				= attrsArray.getColor(R.styleable.CustomButton_fb_textColor,mDefaultTextColor);
        // if default color is set then the icon's color is the same (the default for icon's color)
        mDefaultIconColor               = attrsArray.getColor(R.styleable.CustomButton_fb_iconColor,mDefaultTextColor);

        mDefaultTextSize                = (int) attrsArray.getDimension(R.styleable.CustomButton_fb_textSize, mDefaultTextSize);
        mDefaultTextSize                = (int) attrsArray.getDimension(R.styleable.CustomButton_android_textSize, mDefaultTextSize);

        mDefaultTextGravity             = attrsArray.getInt(R.styleable.CustomButton_fb_textGravity, mDefaultTextGravity);

        mBorderColor 					= attrsArray.getColor(R.styleable.CustomButton_fb_borderColor, mBorderColor);
        mBorderWidth					= (int) attrsArray.getDimension(R.styleable.CustomButton_fb_borderWidth,mBorderWidth);

        mRadius 						= (int)attrsArray.getDimension(R.styleable.CustomButton_fb_radius,mRadius);

        mRadiusTopLeft                  = (int) attrsArray.getDimension(R.styleable.CustomButton_fb_radiusTopLeft, mRadius);
        mRadiusTopRight                 = (int) attrsArray.getDimension(R.styleable.CustomButton_fb_radiusTopRight, mRadius);
        mRadiusBottomLeft               = (int) attrsArray.getDimension(R.styleable.CustomButton_fb_radiusBottomLeft, mRadius);
        mRadiusBottomRight              = (int) attrsArray.getDimension(R.styleable.CustomButton_fb_radiusBottomRight, mRadius);

        mFontIconSize 					= (int)attrsArray.getDimension(R.styleable.CustomButton_fb_fontIconSize, mFontIconSize);

        mIconPaddingLeft                = (int)attrsArray.getDimension(R.styleable.CustomButton_fb_iconPaddingLeft,mIconPaddingLeft);
        mIconPaddingRight               = (int)attrsArray.getDimension(R.styleable.CustomButton_fb_iconPaddingRight,mIconPaddingRight);
        mIconPaddingTop                 = (int)attrsArray.getDimension(R.styleable.CustomButton_fb_iconPaddingTop,mIconPaddingTop);
        mIconPaddingBottom              = (int)attrsArray.getDimension(R.styleable.CustomButton_fb_iconPaddingBottom,mIconPaddingBottom);

        mTextAllCaps                    = attrsArray.getBoolean(R.styleable.CustomButton_fb_textAllCaps, false);
        mTextAllCaps                    = attrsArray.getBoolean(R.styleable.CustomButton_android_textAllCaps, false);

        mGhost                          = attrsArray.getBoolean(R.styleable.CustomButton_fb_ghost, mGhost);
        mUseSystemFont                  = attrsArray.getBoolean(R.styleable.CustomButton_fb_useSystemFont, mUseSystemFont);
        mIsSingleLine                  = attrsArray.getBoolean(R.styleable.CustomButton_fb_isSingleLine, mIsSingleLine);

        String text = attrsArray.getString(R.styleable.CustomButton_fb_text);

        if (text == null) { //no fb_text attribute
            text = attrsArray.getString(R.styleable.CustomButton_android_text);
        }

        mIconPosition 					= attrsArray.getInt(R.styleable.CustomButton_fb_iconPosition,mIconPosition);

        String fontIcon 				= attrsArray.getString(R.styleable.CustomButton_fb_fontIconResource);

        String iconFontFamily 			= attrsArray.getString(R.styleable.CustomButton_fb_iconFont);
        String textFontFamily 			= attrsArray.getString(R.styleable.CustomButton_fb_textFont);

        Drawable icon = null;
        try{
            mIconResource 				= attrsArray.getDrawable(R.styleable.CustomButton_fb_iconResource);
    ...
    public void setTextSize(int textSize){
        this.mDefaultTextSize = PixelUtil.sp2px(getContext(), textSize);
        if(mTextView != null)
    ...
    public void setFontIconSize(int iconSize){
        this.mFontIconSize = PixelUtil.sp2px(getContext(), iconSize);
        if(mFontIconView!=null)
